    ...
    <>
      ...
      <div className="h-[30px] flex items-center justify-between gap-3" data-tauri-drag-region style={{ WebkitAppRegion: "drag" } as React.CSSProperties}>
        ...
        <div className={`flex items-center gap-2 ${platform.type === "tauri" && !isFullscreen ? "pl-[70px]" : ""}`} data-tauri-drag-region>
          <Button variant="ghost" size="icon-sm" onClick={handleClose} title={t("backToHome")} style={{ WebkitAppRegion: "no-drag", cursor: "pointer" } as React.CSSProperties}>
        ...
        <div className="flex items-center gap-1.5">
          ...
          {historyState.canUndo && (
            <Button variant="ghost" size="icon-sm" onClick={undo} title={historyState.undoLabel ? t("undoLabel", { label: historyState.undoLabel }) : t("undo")} style={{ WebkitAppRegion: "no-drag", cursor: "pointer" } as React.CSSProperties}>
              <Undo2 className="w-3.5 h-3.5" />
            </Button>
          )}
          {historyState.canRedo && (
            <Button variant="ghost" size="icon-sm" onClick={redo} title={historyState.redoLabel ? t("redoLabel", { label: historyState.redoLabel }) : t("redo")} style={{ WebkitAppRegion: "no-drag", cursor: "pointer" } as React.CSSProperties}>
              <Redo2 className="w-3.5 h-3.5" />
            </Button>
          )}

          <Button variant="ghost" size="icon-sm" onClick={toggleSettingsModal} title={t("settings")} style={{ WebkitAppRegion: "no-drag", cursor: "pointer" } as React.CSSProperties}>
